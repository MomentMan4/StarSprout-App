import { NextResponse } from "next/server"
import { auth, clerkClient } from "@clerk/nextjs/server"
import { createHousehold, createUser, recordConsents, assignStarterQuest, generateInviteCode } from "@/lib/db"

export const runtime = "nodejs"
export const dynamic = "force-dynamic"

export async function POST(request: Request) {
  try {
    console.log("[v0] Starting parent onboarding - POST route called")

    let userId: string | null = null
    const hasClerkKeys = process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY && process.env.CLERK_SECRET_KEY

    if (hasClerkKeys) {
      const authResult = await auth()
      userId = authResult.userId
      console.log("[v0] Clerk auth userId:", userId)
    } else {
      console.log("[v0] Running in preview mode without Clerk authentication")
    }

    let body
    try {
      body = await request.json()
      console.log("[v0] Received onboarding data:", {
        household_name: body.household_name,
        child_nickname: body.child_nickname,
        clerk_user_id: body.clerk_user_id,
      })
    } catch (err) {
      console.error("[v0] Failed to parse request body:", err)
      return NextResponse.json({ error: "Invalid request body" }, { status: 400 })
    }

    const {
      clerk_user_id,
      household_name,
      parent_nickname,
      child_nickname,
      child_avatar,
      child_age_band,
      consent_coppa,
      consent_ai,
      consent_social,
      starter_quest,
    } = body

    if (!household_name || !child_nickname || !child_age_band) {
      return NextResponse.json({ error: "Missing required fields" }, { status: 400 })
    }

    const effectiveUserId = clerk_user_id || `preview_parent_${Date.now()}`
    console.log("[v0] Effective user ID:", effectiveUserId)

    if (hasClerkKeys && userId && userId !== clerk_user_id) {
      console.error("[v0] User ID mismatch - auth:", userId, "body:", clerk_user_id)
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 })
    }

    // 1. Create household
    console.log("[v0] Creating household:", household_name)
    const household = await createHousehold({ name: household_name })
    if (!household) {
      console.error("[v0] Failed to create household")
      return NextResponse.json({ error: "Failed to create household" }, { status: 500 })
    }
    console.log("[v0] Household created:", household.id)

    // 2. Create parent user
    console.log("[v0] Creating parent user:", effectiveUserId)
    const parent = await createUser({
      id: effectiveUserId,
      household_id: household.id,
      role: "parent",
      nickname: parent_nickname,
    })

    if (!parent) {
      console.error("[v0] Failed to create parent user")
      return NextResponse.json({ error: "Failed to create parent user" }, { status: 500 })
    }
    console.log("[v0] Parent created:", parent.id)

    // 3. Create child user (generate a temp ID for Clerk - parent will create proper account later)
    const childId = `child_${household.id}_${Date.now()}`
    console.log("[v0] Creating child user:", childId)

    const child = await createUser({
      id: childId,
      household_id: household.id,
      role: "child",
      nickname: child_nickname,
      avatar_url: child_avatar,
      age_band: child_age_band as any,
    })

    if (!child) {
      console.error("[v0] Failed to create child user")
      return NextResponse.json({ error: "Failed to create child user" }, { status: 500 })
    }
    console.log("[v0] Child created:", child.id)

    // 4. Record consents
    console.log("[v0] Recording consents")
    const consents = [
      {
        household_id: household.id,
        parent_id: effectiveUserId,
        child_id: childId,
        consent_type: "coppa" as const,
        granted: consent_coppa,
      },
      {
        household_id: household.id,
        parent_id: effectiveUserId,
        child_id: childId,
        consent_type: "ai_features" as const,
        granted: consent_ai || false,
      },
      {
        household_id: household.id,
        parent_id: effectiveUserId,
        child_id: childId,
        consent_type: "social_features" as const,
        granted: consent_social || false,
      },
    ]

    const consentsRecorded = await recordConsents(consents)
    if (!consentsRecorded) {
      console.error("[v0] Failed to record consents")
      return NextResponse.json({ error: "Failed to record consents" }, { status: 500 })
    }
    console.log("[v0] Consents recorded")

    // 5. Assign starter quest
    console.log("[v0] Assigning starter quest:", starter_quest?.title)
    const questAssigned = await assignStarterQuest({
      household_id: household.id,
      assigned_to: childId,
      assigned_by: effectiveUserId,
      title: starter_quest.title,
      category: starter_quest.category,
      points: starter_quest.points,
    })

    if (!questAssigned) {
      console.error("[v0] Failed to assign starter quest")
      return NextResponse.json({ error: "Failed to assign starter quest" }, { status: 500 })
    }
    console.log("[v0] Starter quest assigned")

    // 6. Generate friend invite code for child
    console.log("[v0] Generating invite code")
    const inviteCode = await generateInviteCode({ child_id: childId })
    if (!inviteCode) {
      console.error("[v0] Failed to generate invite code")
      // Non-critical, continue
    } else {
      console.log("[v0] Invite code generated:", inviteCode.code)
    }

    // 7. Update Clerk metadata (only if Clerk is configured)
    if (hasClerkKeys) {
      console.log("[v0] Updating Clerk metadata")
      try {
        const client = await clerkClient()
        await client.users.updateUserMetadata(effectiveUserId, {
          publicMetadata: {
            role: "parent",
            household_id: household.id,
            setup_complete: true,
          },
        })
        console.log("[v0] Clerk metadata updated")
      } catch (clerkError) {
        console.error("[v0] Failed to update Clerk metadata:", clerkError)
        // Non-critical, continue
      }
    } else {
      console.log("[v0] Skipping Clerk metadata update (preview mode)")
    }

    console.log("[v0] Onboarding completed successfully")
    return NextResponse.json(
      {
        success: true,
        household_id: household.id,
        child_id: childId,
        parent_id: effectiveUserId,
      },
      { status: 200 },
    )
  } catch (error: any) {
    console.error("[v0] Parent onboarding error:", error)
    console.error("[v0] Error stack:", error.stack)
    return NextResponse.json(
      {
        error: error.message || "Internal server error",
        details: error.toString(),
      },
      { status: 500 },
    )
  }
}
